...
import { loginWithGmail, loginWithGoogle } from '../services/api';
...
export function GmailAuthModal({
  isOpen,
  onClose,
  onSuccess,
  defaultEmail = '',
  ...
  const handleRealGoogleLogin = async () => {
    ...
    try {
      ...
      setIsLoginNotice({ active: true, isFirst: !!res.user.isFirstLogin });
      setTimeout(() => {
        onSuccess(res.user, res.user.isFirstLogin);
        onClose();
      }, 1200);
    } catch (err: any) {
      setIsGoogleLoading(false);
      ...
      if (msg.includes('unauthorized') || msg.includes('not authorized')) {
        setErrorMsg('This domain is not authorized for Google popup sign-in. Please use the Gmail & Password form below instead.');
      } else {
        setErrorMsg(msg);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanEmail.endsWith('@gmail.com')) {
      setErrorMsg('Strict Policy: Only Google Gmail accounts ending in "@gmail.com" are permitted.');
      return;
    }
    ...
    setIsLoading(true);
    try {
      const res = await loginWithGmail(cleanEmail, passwordInput, nameInput || undefined);
      setIsLoading(false);
      ...
      setIsLoading(false);
      setErrorMsg(err.message || 'Failed to authenticate Gmail account.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
      ...
      >
        ...
        {/* PROMINENT REAL GOOGLE SIGN-IN BUTTON */}
        <div className="mt-5 space-y-3">
          <button
            type="button"
            onClick={handleRealGoogleLogin}
            disabled={isGoogleLoading || isLoading || isLoginNotice.active}
            className="w-full py-3 px-4 rounded-xl bg-white hover:bg-[#E5E5E5] text-zinc-900 font-bold text-xs sm:text-sm transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] flex items-center justify-center gap-3 cursor-pointer active:scale-95 disabled:opacity-50"
          >
            {isGoogleLoading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={isLoading || isGoogleLoading || isLoginNotice.active}
        ...
        <div className="mt-4 pt-3 border-t border-[#222222] text-center text-[10px] text-[#666666] font-mono">
          Lazy UI • Real Google & Firebase Authentication
        </div>
      </motion.div>
    </div>
  );
